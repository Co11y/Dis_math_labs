...
void anti_sym_check(int ** matrix, int len, int len2) {
	// sym_check
	if (len != len2) {
		std::cout << "imposible to define symetricality" << std::endl;
		return;
	...
	for (int i = 1; i < len; i++) {
		for (int k = 1; k < len2; k++) {
			if (i = k) {
				...
				std::cout << "matrix is not anti-symetrical" << std::endl;
				return;
			}
		}
	}
	std::cout << "matrix is anti-symetrical" << std::endl;
	return;
}
				...
				if (matrix[i][j] == 1 && matrix[j][k] == 1 && matrix[i][k] == 1)
{
				...
				{
					std::cout << "matrix is antitransitive" << std::endl;
					return 0;
				}
			}
		}
...
int main() {
	...
	anti_sym_check(matrix, len, len2);
